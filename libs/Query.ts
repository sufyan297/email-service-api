 ...
 * Get Single Record
 */
const _getOne = <T extends ObjectLiteral>(
  modelName: string,
 ...
 * Get Many Records
 */
const _getMany = <T extends ObjectLiteral>(
  modelName: string,
  options: FindManyOptions<unknown> | undefined,
  connectionName?: string,
): Promise<T[]> => {
  return new Promise(async (resolve, reject) => {
    try {
 ...
 * Get Records count
 */
const _getCount = <T extends ObjectLiteral>(
  modelName: string,
  options: FindManyOptions<unknown> | undefined,
  connectionName?: string,
): Promise<number> => {
 ...
 * Get Pagination
 */
const _getPagination = <T extends ObjectLiteral>(
  modelName: string,
 ...
 */
const _saveData = <T extends ObjectLiteral>(
  modelName: string,
  fields: any,
  connectionName?: string,
): Promise<T | T[]> => {
      ...
      if (typeof value == "object" && !Array.isArray(value) && value != null) {
        ...
        if (hasSpace(key)) {
          ...
          if (field && field[0] && field[1]) {
            ...
            switch (operator) {
              case "!=":
                newWhere = { ...newWhere, [field[0]]: Not(value) };
                break; //"id !=": "1"
              case "IN":
                newWhere = { ...newWhere, [field[0]]: In(value) };
                break; // "id IN": ["1","2"]
              case "NOT_IN":
                newWhere = { ...newWhere, [field[0]]: Not(In(value)) };
                break; //"id NOT_IN": ["1"]
              case ">":
                newWhere = { ...newWhere, [field[0]]: MoreThan(value) };
                break;
              case ">=":
                newWhere = { ...newWhere, [field[0]]: MoreThanOrEqual(value) };
                break;
              case "<":
                newWhere = { ...newWhere, [field[0]]: LessThan(value) };
                break;
              case "<=":
                newWhere = { ...newWhere, [field[0]]: LessThanOrEqual(value) };
                break;
              case "LIKE":
                newWhere = { ...newWhere, [field[0]]: Like(value) };
                break; //"id LIKE": ["1"]
              case "NOT_LIKE":
                newWhere = { ...newWhere, [field[0]]: Not(Like(value)) };
                break; //"id NOT_LIKE": ["1"]
              case "RAW":
                newWhere = { ...newWhere, [field[0]]: Raw(value) };
                break; //"first_name RAW": "REGEXP "
              case "BETWEEN": {
