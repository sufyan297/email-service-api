...
const Login = async ({
  ...
  try {
    ...
    if (!admin) throw new ResourceNotFoundException("Cannot Find Admin");

    const match = await compare(password, admin.password);

    if (!match) throw new UnAuthorizedAccessException("Credentials Did Not Match");
...
const CreateAdmin = async (body: Partial<Admin>, user: Admin): Promise<APIResponse<Admin>> => {
  try {
    ...
    if (existingUsername > 0) throw new AppError("Username Already Exists", 409);
...
const UpdatePassword = async (
  ...
  try {
    ...
    if (!match) throw new BadRequestException("Old Password Did Not Match");
...
export { Login, CreateAdmin, UpdateAdmin, UpdatePassword };
